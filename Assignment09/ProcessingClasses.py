...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        Returns:
            ...
            None: returns None if CD object was not found

        """
        try:
            cd_idx = int(cd_idx)
        except ValueError:
            raise ValueError(f'CD ID {cd_idx} in not an integer!')
        for cd in table:
            if cd.cd_id == cd_idx:
                return cd
        return None  # returns None if CD wasn't found
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        """
        track_id, track_title, track_length = track_info
        try:
            track_id = int(track_id)
        except ValueError:
            raise ValueError(f'Track ID {track_id} is not an integer!')
        track = DC.Track(track_id, track_title, track_length)
        cd.add_track(track)
