    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            with open(file_name_CD, 'w') as file:
                ...
                raise Exception(f'Failed to save CDs to {file_name_CD}!')
        try:
                ...
                for cd in lst_Inventory: 
                    ...
                    for track in tracks: 
                        if track is not None: 
                            file.write(f'{cd_id},{track.get_record()}')
        except Exception:
            raise Exception(f'Failed to save tracks to {file_name_tracks}!')
        ...
        try:
            with open(file_name_CD, 'r') as file:
                for line in file:
                    ...
                    row = DC.CD(cd_id, cd_title, cd_artist)
                    lst_Inventory.append(row)
        except Exception:
            raise Exception(f'Could not load CDs from {file_name_CD}')
        try:
            with open(file_name_tracks, 'r') as file: 
                for line in file: 
                    cd_id, track_id, track_title, track_length = line.strip().split(',')
                    cd = PC.DataProcessor.select_cd(lst_Inventory, int(cd_id)) 
                    track = DC.Track(int(track_id), track_title, track_length) 
                    cd.add_track(track)
        except Exception:
            raise Exception(f'Failed to load tracks from {file_name_tracks}!')
